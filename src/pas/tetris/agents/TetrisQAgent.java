    ...
    extends QAgent
{
    ...
    public Model initQFunction()
    {
        ...
        // return qFunction;
        final int numFeatures = Board.NUM_COLS * 5 +1; 
                                    ...
                                    final Mino potentialAction)
    {
        ...
        Matrix featureVector = Matrix.zeros(1, numCols * 5 + 1); // +1 to store the overall bumpiness across all columns
        int index = 0;
        int[] columnHeights = new int[numCols];
        ...
        for (int col = 0; col < numCols; col++) {
            ...
            for (int row = 0; row < numRows; row++) {
                ...
                if (seenBlock && tetrisBoardMatrix.get(row, col)  == 0){ // found potential space for a hole between block
                    numHolesInCol ++;
                    seenBlock = false;
                }
            }
            ...
            int columnHeight = firstMinoPosition != -1 ? numRows - firstMinoPosition : 0;
            columnHeights[col] = columnHeight;
            ...
            featureVector.set(0, index++, columnHeight);   // Add the column height
            featureVector.set(0, index++, numHolesInCol);
        }

        // Calculate bumpiness: calculated as the sum of the absolute differences between adjacent column heights. A smoother board will have lower bumpiness.
        int bumpiness = 0;
        for (int i = 0; i < numCols - 1; i++) {
            bumpiness += Math.abs(columnHeights[i] - columnHeights[i + 1]);
        }
        featureVector.set(0, index++, bumpiness);  

        // System.out.println("featureVector " +featureVector);  
        // System.out.println("featureVector size " +featureVector.getShape()); 
        // System.out.println("potential action type " + potentialAction.getType()); 
    ...
    public double getReward(final GameView game)
    {
        ...
        // System.out.println("-----------------------------------------------------------------------------------");
        // System.out.println("Score this turn: " + game.getTotalScore());
